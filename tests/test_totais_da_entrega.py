...
def _conferir(caminho: Path) -> str:
    ...
    # Balancete COSIF (banco): a entrega é montada pelos TOTAIS de grupo (de-para
    # de domínio), não pelo rollup de cada folha. Quando o balanço fecha
    # (``balanco_confere``), divergências de subtotal no meio da árvore são ruído
    # da origem, não inconsistência da entrega — e o aviso é uma nota, não o
    # alarme "não fecha". É um estado válido novo, distinto dos de baixo.
    if getattr(resultado, "cosif", False) and resultado.balanco_confere:
        return "conferido"
